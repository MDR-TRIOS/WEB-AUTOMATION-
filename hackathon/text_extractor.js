...
class TextExtractor {
    ...
    showExtractionButton() {
        ...
        button.style.cssText = `
            ...
            width: 220px;
            padding: 14px 24px;
            background: linear-gradient(135deg, #ffb347 0%, #ff5e62 50%, #2c5282 100%);
            color: #fff;
            ...
            cursor: pointer;
            box-shadow: 0 8px 32px rgba(44,83,130,0.25), 0 2px 8px rgba(255,94,98,0.15);
            z-index: 99999;
            ...
            text-shadow: 0 2px 8px rgba(44,83,130,0.12);
            border-bottom: 4px solid #ff5e62;
        `;

        button.addEventListener('mouseenter', (e) => {
            if (!e.target) return;
            e.target.style.background = 'linear-gradient(135deg, #ff5e62 0%, #7f53ac 100%)';
            e.target.style.transform = 'scale(1.10)';
            e.target.style.boxShadow = '0 12px 36px rgba(127,83,172,0.25), 0 4px 16px rgba(255,179,71,0.18)';
        }, { passive: true });

        button.addEventListener('mouseleave', (e) => {
            if (!e.target) return;
            e.target.style.background = 'linear-gradient(135deg, #ffb347 0%, #ff5e62 50%, #2c5282 100%)';
            e.target.style.transform = 'scale(1)';
            e.target.style.boxShadow = '0 8px 32px rgba(44,83,130,0.25), 0 2px 8px rgba(255,94,98,0.15)';
        }, { passive: true });
        ...
        button.addEventListener('click', async (e) => {
            ...
            try {
                ...
                if (btn) btn.remove();
                // Log a unique message for backend detection
                console.log('[OPEN_FIREFOX]');
            } catch (err) {
